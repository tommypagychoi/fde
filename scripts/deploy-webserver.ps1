...
& $SshExe -i $SshKey -p $RemotePort "${User}@${RemoteHost}" "mkdir -p /var/www/fde && mv /tmp/index.html /var/www/fde/index.html && mv /tmp/fde.conf /etc/nginx/sites-available/fde.conf && rm -f /etc/nginx/sites-enabled/default && ln -sf /etc/nginx/sites-available/fde.conf /etc/nginx/sites-enabled/fde.conf && nginx -t && systemctl reload nginx"
